 ...
 * @functions:
 */
class confkeyController extends privilege{
	/**
	 * @var confkeyView
	 */
	protected $view;
...
//		var_dump($this->model->getData($path));exit;
	...
	public function addAction(message $msg){
		...
		$this->view->add(
			$msg["?path"], 
			ENTRY_HOME."/priv/confkey/append",
			$this->model->defaultValue()
		);
	}
	public function appendAction(message $msg){
//		$this->view->_404();
		if(!isset($msg["path"],$msg["type"],$msg["key"],$msg["comment"])){
			$this->view->_404();
			return ;
		}
		try{
			$this->model->append($msg["path"],$msg["type"],$msg["key"],$msg["comment"]);
			return $this->view->showOk("添加成功",ENTRY_HOME."/priv/confkey?path=".urlencode($msg["path"]."?"));
		}catch (Exception $e){
			$this->view->showFail($e);
		}
	}
...
//		$this->view->_404();
		if(!isset($msg["path"],$msg["key"],$msg["comment"])){
			$this->view->_404();
			return ;
		}
		try{
			$this->model->update($msg["path"],$msg["key"],$msg["comment"]);
...
//		var_dump($data);exit;
		...
		$this->view->edit(
			$msg["?path"], 
			ENTRY_HOME."/priv/confkey/update",
			$data
		);
	}
