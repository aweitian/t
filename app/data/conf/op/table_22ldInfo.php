...
class table_22ldInfo{
	...
	public function getConf(){
		...
		$row = $this->pdo->fetch("select `mutistyle`,`unitprice`,`tableCaption`,`showType`,`gridCol`,`span`,`spanNum`,`titleType`,`showCalcData`,`showZero2End`,`showA2B`,`comment` from `tny_fs_conf_table_22ld` where `csid`=:csid", array(
			"csid"=>$row["sid"]
		));
		return $row;
	}	
