...
class table_23tpeInfo{
	...
	public function getConf(){
		...
		$row = $this->pdo->fetch("select `mutistyle`,`tableCaption`,`showType`,`gridCol`,`titleType`,`extraShowMode`,`iconPlaceHolder`,`extraShowMask`,`extraCaption`,`comment` from `tny_fs_conf_table_23tpe` where `csid`=:csid", array(
			"csid"=>$row["sid"]
		));
		return $row;
	}	
