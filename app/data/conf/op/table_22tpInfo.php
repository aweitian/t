...
class table_22tpInfo{
	...
	public function getConf(){
		...
		$row = $this->pdo->fetch("select `mutistyle`,`tableCaption`,`showType`,`gridCol`,`titleType`,`comment` from `tny_fs_conf_table_22tp` where `csid`=:csid", array(
			"csid"=>$row["sid"]
		));
		return $row;
	}	
