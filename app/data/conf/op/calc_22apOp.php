 ...
 * function: 
 */
require_once dirname(__FILE__)."/calc_22apOpRegexp.php";
...
// 		var_dump($data);exit();
	...
	public function remove($path){
		...
		$ret = $this->pdo->insert("delete from `tny_fs_conf_calc_22ap` where `csid` = :csid", array(
			"csid" => $sid,
		));
		if(!$ret){
			$info = $this->pdo->getErrorInfo();
	...
	public function update($path,$data){
		...
		$sid = $this->dbhelper->getConfsidByPath($path);
// 		var_dump($sid);exit;
		$ret = $this->pdo->exec("update `tny_fs_conf_calc_22ap` set 
				`cachejs` = :cachejs,	
				`comment` = :comment,	
				`unit` = :unit	
