...
class datasrc_ofhintInfo{
	...
	/**
	 * 
	 * @return DataSrc_ofhint
	 * @param unknown_type $dsnsid
	 * @throws Exception
	 */
	public function getData(){
		$row = $this->pdo->fetchAll("SELECT `t` FROM `tny_fs_datasrc_ofhint`
			LEFT JOIN `tny_fs_ns_struct` ON `tny_fs_ns_struct`.`sid` = `tny_fs_datasrc_ofhint`.`nssid` AND `tny_fs_ns_struct`.`nt` = 'file'
			...
			ORDER BY `tny_fs_datasrc_ofhint`.`t` ASC",
		array(
			"nodepath" => $this->path->getNodePath(),
			"datakey" => $this->path->getDatakey(),
			"nspath" => $this->path->getNsPath(),
		),PDO::FETCH_NUM);
// 		var_dump($this->path->getNodePath(),$this->path->getDatakey(),$this->path->getNsPath());exit;
		$ret = array();
		foreach ($row as $item){
			$ret[] = $item[0];
		}
	...
	public static function isCutInheritLinkArr($arr){
		return is_array($arr) && count($arr) == 1 && $arr[0] == "-1";
	}
}