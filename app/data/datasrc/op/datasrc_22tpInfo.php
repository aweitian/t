...
class datasrc_22tpInfo{
	...
	public function getData(){
		$row = $this->pdo->fetchAll("SELECT `t`,`p` FROM `tny_fs_datasrc_22tp`
			LEFT JOIN `tny_fs_ns_struct` ON `tny_fs_ns_struct`.`sid` = `tny_fs_datasrc_22tp`.`nssid` AND `tny_fs_ns_struct`.`nt` = 'file'
			...
			ORDER BY `tny_fs_datasrc_22tp`.`order` ASC",
		array(
