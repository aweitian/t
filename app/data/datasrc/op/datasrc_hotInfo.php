...
class datasrc_hotInfo{
	...
	public function getData(){
		$row = $this->pdo->fetchAll("SELECT `t`,`op`,`np`,`ico`,`tny_fs_datasrc_hot`.`order` as `order`,
				`tny_fs_datasrc_hot`.`otpl` as `otpl` FROM `tny_fs_datasrc_hot`
			LEFT JOIN `tny_fs_ns_struct` ON `tny_fs_ns_struct`.`sid` = `tny_fs_datasrc_hot`.`nssid` AND `tny_fs_ns_struct`.`nt` = 'file'
