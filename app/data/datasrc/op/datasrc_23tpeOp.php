...
class datasrc_23tpeOp{
	...
	public function add($path,$data){
		if(is_array($data) && datasrc_23tpeOpRegexp::isValid($data)){
		...
		foreach ($data as $item){
			$id = $this->pdo->insert("insert into `tny_fs_datasrc_23tpe`
			(`nssid`,`t`,`p`,`e`,`order`) values (:nssid,:t,:p,:e,:order)", array(
				"nssid" => $row['sid'],
				"t" => $item[0],
				"p" => $item[1],
				"e" => $item[2],
	...
	private function removeByNssid($nssid){
		return $this->pdo->exec("delete from tny_fs_datasrc_23tpe where nssid=:nssid", array(
			"nssid"=>$nssid
		));
	}
	...
	public function update($path,$data){
		...
		foreach ($sidArr as $a => $sid){
			if(array_key_exists($a, $data)){
				$c += $this->pdo->exec("update tny_fs_datasrc_23tpe set `p`=:p,`order`=:order,`e`=:e where sid=:sid and `t`=:t", array(
					"t"=>$a,
					"p"=>$data[$a][0],
					"e"=>$data[$a][1],
					"order"=>$data[$a][2],
		...
		foreach ($data as $a => $item){
			if(!is_null(key($sidArrBak))){
				...
				$sid = $sidArrBak[$key];
				var_dump($sid);
				$c += $this->pdo->exec("update tny_fs_datasrc_23tpe set `p`=:p,`t`=:t,`order`=:order,`e`=:e where sid=:sid", array(
					"t"=>$a,
					"p"=>$item[0],
					"e"=>$item[1],
					"order"=>$item[2],
		...
		foreach ($databak as  $a => $item){
			$c += $this->pdo->exec("insert into tny_fs_datasrc_23tpe (`nssid`,`t`,`p`,`e`,`order`) values (:nssid,:t,:p,:e,:order)", array(
				"nssid"=>$nssid,
				"t"=>$a,
				"p"=>$item[0],
				"e"=>$item[1],
		...
		foreach ($sidArrBak as $sid){
			$c += $this->pdo->exec("delete from tny_fs_datasrc_23tpe where sid=:sid", array(
				"sid"=>$sid,
			));
		}
		return $c;
	...
	private function _convertAmountBase($data){
		$ret = array();
		foreach ($data as $item){
			$ret[$item[0]] = array($item[1],$item[2],$item[3]);
		}
		return $ret;
	}
	private function _getSidArrByNssid($nssid){
		$row = $this->pdo->fetchAll("select `t`,`sid` from tny_fs_datasrc_23tpe where nssid=:nssid order by `t` asc", array(
			"nssid"=>$nssid,
		));
