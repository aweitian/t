...
class datasrc_22ldOp{
	...
	public function add($path,$data){
		if(is_array($data) && datasrc_22ldOpRegexp::isValid($data)){
		...
		foreach ($data as $item){
			$id = $this->pdo->insert("insert into `tny_fs_datasrc_22ld`
			(`nssid`,`l`,`d`) values (:nssid,:l,:d)", array(
				"nssid" => $row['sid'],
				"l" => $item[0],
				"d" => $item[1],
			));
	...
	private function removeByNssid($nssid){
		return $this->pdo->exec("delete from tny_fs_datasrc_22ld where nssid=:nssid", array(
			"nssid"=>$nssid
		));
	}
	...
	public function update($path,$data){
		...
		foreach ($sidArr as $a => $sid){
			if(array_key_exists($a, $data)){
				$c += $this->pdo->exec("update tny_fs_datasrc_22ld set `d`=:d where sid=:sid and `l`=:l", array(
					"l"=>$a,
					"d"=>$data[$a][0],
					"sid"=>$sid,
				));
		...
		foreach ($data as $a => $item){
			if(!is_null(key($sidArrBak))){
				...
				$c += $this->pdo->exec("update tny_fs_datasrc_22ld set `d`=:d,`l`=:l where sid=:sid", array(
					"l"=>$a,
					"d"=>$item[0],
					"sid"=>$sid,
				));
		...
		foreach ($databak as  $a => $item){
			$c += $this->pdo->exec("insert into tny_fs_datasrc_22ld (`nssid`,`l`,`d`) values (:nssid,:l,:d)", array(
				"nssid"=>$nssid,
				"l"=>$a,
				"d"=>$item[0],
			));
		...
		foreach ($sidArrBak as $sid){
			$c += $this->pdo->exec("delete from tny_fs_datasrc_22ld where sid=:sid", array(
				"sid"=>$sid,
			));
		}
		return $c;
	...
	private function _getSidArrByNssid($nssid){
		$row = $this->pdo->fetchAll("select `l`,`sid` from tny_fs_datasrc_22ld where nssid=:nssid order by `l` asc", array(
			"nssid"=>$nssid,
		));
