...
class DataSrc_23tpe extends ADataSrc implements IOrder{
	...
	public static function check($data){
		...
		for($i=0; $i<count($data) && $f; $i++){
			...
			&& is_string($data[$i][0]) && is_string($data[$i][2]) && is_numeric($data[$i][1]);
		}
		return $f;
	}
	public static function checkWithOrder($data){
		...
		$f = $f && util::isDigitalSeqArray($data);
		for($i=0; $i<count($data) && $f; $i++){
			$f = is_array($data[$i]) && count($data[$i]) == 4 
			&& util::isDigitalSeqArray($data[$i])
			&& is_string($data[$i][0]) && is_string($data[$i][2]) 
			&& is_numeric($data[$i][1]) && is_numeric($data[$i][3]);
		}
		return $f;
	}
}